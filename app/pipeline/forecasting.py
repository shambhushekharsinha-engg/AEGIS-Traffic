...
class TimeSeriesForecastEngine:
    ...
    def generate_timeline_forecast(
        self, current_density: float = 62.5, location_name: str = "Connaught Place"
    ) -> dict:
        """
        ...
        horizons = [
            {"label": "Now", "offset_min": 0, "multiplier": 1.0, "confidence_std": 2.0},
            {
                "label": "+15 min",
                "offset_min": 15,
                "multiplier": 1.05 if diurnal_factor > 0.6 else 0.95,
                "confidence_std": 4.5,
            },
            {
                "label": "+30 min",
                "offset_min": 30,
                "multiplier": 1.12 if diurnal_factor > 0.6 else 0.88,
                "confidence_std": 7.0,
            },
            {
                "label": "+1 hour",
                "offset_min": 60,
                "multiplier": 1.20 if diurnal_factor > 0.6 else 0.75,
                "confidence_std": 10.5,
            },
            {
        ...
        for h in horizons:
            pred_density = min(
                ...
                max(
                    10.0,
                    round(base_val * h["multiplier"] + np.random.uniform(-1.5, 1.5), 1),
                ),
            )
            ...
            timeline.append(
                {
                    ...
                    "status_level": level,
                    "projected_avg_speed_kmh": round(
                        60.0 / (1.0 + 0.03 * (pred_density / 2.0)), 1
                    ),
                }
            )
