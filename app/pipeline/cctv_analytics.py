...
class CCTVAnalyticsEngine:
    def __init__(self):
        self.supported_classes = ["car", "truck", "bus", "motorcycle", "bicycle", "pedestrian"]
        self.active_cameras = {
            "CAM-01": {"name": "Connaught Place North Junction", "location": "Connaught Place", "status": "ONLINE", "fps": 29.8, "lat": 28.6315, "lng": 77.2167},
            "CAM-02": {"name": "Outer Ring Road - Cyber City", "location": "Gurugram", "status": "ONLINE", "fps": 30.0, "lat": 28.4595, "lng": 77.0266},
            "CAM-03": {"name": "Noida Expressway Toll Plaza", "location": "Noida", "status": "ONLINE", "fps": 25.4, "lat": 28.5355, "lng": 77.3910},
            "CAM-04": {"name": "AIIMS Flyover Hub", "location": "South Delhi", "status": "ONLINE", "fps": 28.2, "lat": 28.5672, "lng": 77.2100},
            "CAM-05": {"name": "Indirapuram Crossing", "location": "Ghaziabad", "status": "OFFLINE", "fps": 0.0, "lat": 28.6410, "lng": 77.3712},
        }

    def process_cctv_frame(self, camera_id: str = "CAM-01", custom_frame_data: dict = None) -> dict:
        """
        ...
        tracks = []
        track_id = 100
        for cat, count in [("car", cars), ("truck", trucks), ("bus", buses), ("motorcycle", motorcycles)]:
            for i in range(count):
                ...
                speed_kmh = max(5.0, round(60.0 / (1.0 + 0.04 * total_vehicles) + np.random.uniform(-3, 3), 1))
        ...
        avg_speed = round(float(np.mean([tr["speed_kmh"] for tr in tracks])) if tracks else 45.0, 1)
        ...
        return {
            ...
            "analytics": {
                ...
                "congestion_level": "High" if congestion_index > 75 else ("Medium" if congestion_index > 40 else "Low"),
    ...
    def list_cameras(self) -> list:
        return [
            {"id": cid, **cinfo} for cid, cinfo in self.active_cameras.items()
        ]
