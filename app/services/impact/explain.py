...
class ExplainabilityEngine:
    def generate_explanation(self, impact_metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        observed = [m for m in impact_metrics.get("metrics", []) if m["evidence_type"] == "observed"]
        estimated = [m for m in impact_metrics.get("metrics", []) if m["evidence_type"] == "estimated"]
        
        queue_metric = next((m for m in estimated if m["metric"] == "estimated_queue"), None)
        speed_metric = next((m for m in observed if m["metric"] == "average_speed"), None)
        count_metric = next((m for m in observed if m["metric"] == "vehicle_count"), None)
        ...
        return {
            "title": f"🚦 Intervention Recommended" if severity != "LOW" else "🟢 Status Normal",
            "severity": severity,
            "reasons": reasons,
