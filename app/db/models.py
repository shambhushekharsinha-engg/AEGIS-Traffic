...
class IncidentLog(Base):
    ...
    violations = relationship(
        "ViolationRecord", back_populates="incident", cascade="all, delete-orphan"
    )
...
class ViolationRecord(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    incident_id = Column(
        Integer, ForeignKey("incident_logs.id", ondelete="CASCADE"), nullable=False
    )
...
class CitizenHazardReport(Base):
    ...
    )  # Pothole | Accident | Signal Outage | Flooding | Debris
    ...
    )  # SUBMITTED | IN_REVIEW | DISPATCHED | RESOLVED
    created_at = Column(DateTime, nullable=False, default=datetime.utcnow, index=True)
