...
def seed_default_users(db: Session) -> None:
    ...
    defaults = [
        ...
        {
            "username": "operator",
            "password": "Operator@AEGIS2024!",
            "role": "Operator",
            "full_name": "Traffic Operator",
        },
        {
            "username": "auditor",
            "password": "Auditor@AEGIS2024!",
            "role": "Auditor",
...
def get_incidents(
    ...
    if scenario:
        q = q.filter(IncidentLog.scenario == scenario.upper())
    if operator_name:
        q = q.filter(IncidentLog.operator_name == operator_name)
    if priority:
        q = q.filter(IncidentLog.priority.contains(priority))
    total = q.count()
...
def get_violations(
    ...
    if plate:
        q = q.filter(ViolationRecord.plate.contains(plate))
    if type_code:
        q = q.filter(ViolationRecord.type_code == type_code)
    if severity:
        q = q.filter(ViolationRecord.severity == severity)
    total = q.count()
...
def get_incident_stats(db: Session) -> dict:
    ...
    by_scenario = (
        ...
        .group_by(IncidentLog.scenario)
        .all()
    )
    by_priority = (
        db.query(IncidentLog.priority, func.count(IncidentLog.id))
        .group_by(IncidentLog.priority)
        .all()
    )
...
def get_citizen_hazard_reports(
    ...
    total = q.count()
    items = (
        q.order_by(desc(CitizenHazardReport.created_at)).offset(skip).limit(limit).all()
    )
    return items, total
