...
class FolderStreamAnalyzer:
    def __init__(self):
        ...
        if CV2_AVAILABLE and ULTRALYTICS_AVAILABLE and YOLO is not None:
            try:
                ...
                print(
                    f"⚠️ Failed to load YOLOv8 model: {e}. Running in simulation fallback mode."
                )
        else:
            if not CV2_AVAILABLE:
                print(
                    "⚠️ cv2 not available (Vercel env). Running in pure-simulation mode."
                )
            elif not ULTRALYTICS_AVAILABLE:
                print(
                    "⚠️ YOLOv8 model not available (ultralytics not installed). Running in simulation fallback mode."
                )
    ...
    def generate_synthetic_traffic_frame(self, scenario: str):
        ...
        if scenario == "normal":
            ...
            self._draw_vehicle(
                img, (290, 200), (45, 45), (100, 100, 100), "Car (Crashed)"
            )
            self._draw_vehicle(
                img, (325, 225), (45, 45), (200, 50, 50), "Car (Crashed)"
            )
            points = np.array(
                [
                    [315, 200],
            ...
            self._draw_vehicle(
                img, (380, 390), (35, 55), (120, 120, 120), "Car (Stopped)"
            )
            self._draw_vehicle(
                img, (100, 275), (55, 35), (80, 150, 80), "Car (Stopped)"
            )
            ...
            cv2.putText(
                img,
                "FEED LOSS: CAMERA OBSCURED / HARDWARE TAMPER",
                (50, 240),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (0, 0, 255),
    ...
    def _draw_vehicle(self, img, pos, size, color, label, outline_color=None):
        ...
        if w > h:
            cv2.rectangle(
                img, (x + w - 15, y + 3), (x + w - 5, y + h - 3), (230, 230, 230), -1
            )
        else:
            cv2.rectangle(img, (x + 3, y + 5), (x + w - 3, y + 15), (230, 230, 230), -1)

        cv2.putText(
            img, label, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1
        )
    ...
    def process_traffic_scene(self, scenario: str):
        ...
        if self.model_loaded and scenario != "tamper":
            try:
                ...
                for box in results.boxes:
                    ...
                    if (
                        label in ["car", "truck", "bus", "motorcycle", "person"]
                        and confidence > 0.35
                    ):
                ...
                print(
                    f"⚠️ YOLO inference failed, using pure synthetic overlay. Error: {e}"
                )
        ...
        if not detections:
            for item in ground_truth:
                ...
                color = (
                    (0, 0, 255)
                    if "TAMPER" in item["label"] or "Crashed" in item["label"]
                    else (0, 255, 0)
                )
