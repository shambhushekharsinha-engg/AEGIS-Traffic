    ...
    def _generate_synthetic_wav(self, scenario: str, file_path: str):
        ...
        if scenario == "normal":
            ...
            phase = (
                ...
                * (
                    900 * t
                    + (300 / (2 * np.pi * 0.75)) * (1 - np.cos(2 * np.pi * 0.75 * t))
                )
            )
        ...
        print(
            f"🎵 Synthesized {scenario.upper()} traffic sound to {file_path} (Pure Wave)"
        )
    ...
    def check_anomaly(self, audio_path: str):
        ...
        try:
            ...
            try:
                ...
                waveform = (
                    np.random.randn(1, int(self.sample_rate * self.duration_seconds))
                    * 0.01
                )
                sr = self.sample_rate
        ...
        fft_amp_pts = (
            amps_limited[::step_fft][:100] / (amps_limited.max() + 1e-6)
        ).tolist()  # Normalize
