...
class EnvironmentalTelemetryCore:
    ...
    def calculate_emissions(
        ...
        for vtype, cnt in type_counts.items():
            factors = EMISSION_FACTORS_G_PER_MIN.get(vtype, EMISSION_FACTORS_G_PER_MIN["default"])
        ...
        baseline_signal_sec = signal_timing_seconds * 1.4 if atsc_enabled else signal_timing_seconds
        baseline_co2_g = (baseline_signal_sec / 60.0) * (total_co2_g / max(idle_minutes, 0.001))
        ...
        return {
            ...
            "air_quality_index_impact": "GOOD" if total_pm25_g < 0.05 else ("MODERATE" if total_pm25_g < 0.15 else "UNHEALTHY_FOR_SENSITIVE"),
