...
def test_privacy_redaction_removes_pii():
    ...
    assert redacted["plate_number"] is None, "Plate number must be None when privacy is enabled."
    assert redacted["plate_confidence"] is None, "Plate confidence must be None when privacy is enabled."
    assert redacted["face_id"] is None, "Face ID must be None when privacy is enabled."
    assert redacted.get("privacy_mode") is True, "Telemetry must be flagged as privacy_mode=True."
...
def test_privacy_disabled_preserves_pii():
    ...
    assert preserved["plate_number"] == "XYZ-1234", "Plate number must be preserved when privacy is disabled."
...
def test_privacy_idempotency():
    ...
    assert redacted_once == redacted_twice, "Redact should be idempotent: redact(redact(data)) == redact(data)"
