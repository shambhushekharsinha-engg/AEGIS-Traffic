...
def approve_decision(decision_id: str, req: ReviewRequest, db: Session = Depends(get_db)):
    decision = db.query(DecisionRecord).filter(DecisionRecord.decision_id == decision_id).first()
    if not decision:
        raise HTTPException(status_code=404, detail="Decision not found")
        
    if decision.status != "PENDING":
        raise HTTPException(status_code=400, detail="Only pending decisions can be reviewed. Create a new event to override.")
...
def reject_decision(decision_id: str, req: ReviewRequest, db: Session = Depends(get_db)):
    decision = db.query(DecisionRecord).filter(DecisionRecord.decision_id == decision_id).first()
    if not decision:
        raise HTTPException(status_code=404, detail="Decision not found")
...
def get_history(db: Session = Depends(get_db)):
    return db.query(DecisionRecord).order_by(DecisionRecord.created_at.desc()).limit(50).all()
