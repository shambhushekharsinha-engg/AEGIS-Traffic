...
def dispatch_uav(
    request: DroneDispatchRequest, user=Depends(require_role(["Admin", "Operator"]))
):
    """
...
def anchor_to_ledger(
    request: BlockchainAnchorRequest, user=Depends(require_role(["Admin", "Auditor"]))
):
    """
...
def v2i_green_wave(request: V2IPreemptRequest):
    ...
    if request.vehicle_type not in ["AMBULANCE", "FIRE", "POLICE"]:
        raise HTTPException(
            status_code=403, detail="Preemption restricted to emergency vehicles"
        )
