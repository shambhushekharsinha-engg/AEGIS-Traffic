...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.markdown(
            '<div class="t-section" style="font-size:.78rem;margin:10px 0 8px 4px;color:#00f0ff;letter-spacing:2px;">⚡ SYSTEM MODULES</div>',
            unsafe_allow_html=True,
        )
        ...
        for page_name, display_name in MODULE_LIST:
            ...
            if st.button(
                btn_label, key=f"sb_3d_btn_{display_name}", use_container_width=True
            ):
        ...
        st.markdown(
            '<div class="t-section" style="font-size:.72rem;margin-bottom:8px;padding-left:4px;">SIMULATION & OVERRIDE</div>',
            unsafe_allow_html=True,
        )
