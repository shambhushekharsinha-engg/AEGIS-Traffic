...
def metric_tile(
    label: str, value: str, unit: str = "", color: str = "#00f0ff", icon: str = ""
) -> str:
...
def render_login_portal(client, on_login_success):
    ...
    with col_c:
        ...
        with tab_login:
            ...
            if col_q1.button(
                "🔑 Admin Quick Entry", use_container_width=True, key="btn_quick_admin"
            ):
            ...
            default_u = st.session_state.get(
                "login_username_input", st.session_state.get("quick_user", "")
            )
            default_p = st.session_state.get(
                "login_password_input", st.session_state.get("quick_pass", "")
            )

            with st.form("login_form", clear_on_submit=False):
                st.markdown(
                    '<div class="t-section" style="margin-bottom:14px;">Operator Credentials</div>',
                    unsafe_allow_html=True,
                )
                ...
                submitted = st.form_submit_button(
                    "🔐  AUTHENTICATE & CONNECT", use_container_width=True
                )
        ...
        with tab_register:
            ...
            with st.form("register_form", clear_on_submit=True):
                st.markdown(
                    '<div class="t-section" style="margin-bottom:14px;">New Operator Registration</div>',
                    unsafe_allow_html=True,
                )
                ...
                reg_btn = st.form_submit_button(
                    "📋  REGISTER CREDENTIALS", use_container_width=True
                )
                ...
                if reg_btn:
                    if not reg_user or not reg_pass:
                        ...
                        try:
                            ...
                            st.success(
                                f"✅ {reg_user} registered as {reg_role}. Login to access."
                            )
                        except Exception as e:
                            st.error(f"❌ {e}")
