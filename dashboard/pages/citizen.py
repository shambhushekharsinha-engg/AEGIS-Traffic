...
def render_citizen_page(client):
    ...
    with cit_tab1:
        ...
        with st.form("citizen_incident_form", clear_on_submit=True):
            inc_type = st.selectbox("Incident Type", ["Vehicle Collision / Accident", "Broken Traffic Light Signal", "Pothole / Road Obstruction", "Waterlogging / Flooding", "Illegal Parking Barrier"])
            inc_loc = st.text_input("Location / Intersection Name", value=st.session_state.get("location_name", "Connaught Place, New Delhi"))
            inc_desc = st.text_area("Detailed Description", placeholder="Describe the hazard or traffic obstacle...")
            inc_submit = st.form_submit_button("🚨 SUBMIT MUNICIPAL INCIDENT REPORT", use_container_width=True)
            ...
            if inc_submit:
                if not inc_desc:
                    ...
                    try:
                        res = requests.post(f"{BACKEND}/api/v1/citizen/report", json={
                            "type": inc_type,
                            "location": inc_loc,
                        ...
                        if res.ok:
                            st.success("✅ Incident report submitted successfully to Municipal Dispatch!")
                        else:
    ...
    with cit_tab2:
        ...
        with st.form("crosswalk_request_form", clear_on_submit=True):
            ped_count = st.number_input("Estimated Pedestrians Crossing", min_value=1, max_value=100, value=5)
            group_type = st.selectbox("Group Type", ["Standard Pedestrians", "School Children Group", "Senior Citizens / Elderly", "Visually / Physically Impaired"])
            cw_submit = st.form_submit_button("🚶 REQUEST EXTENDED WALK TIMING", use_container_width=True)

            if cw_submit:
                st.success(f"✅ Pedestrian Crosswalk request received for {ped_count} ({group_type}). Signal timing extended!")
