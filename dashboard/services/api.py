    ...
    def __init__(
        ...
        if detail:
            if isinstance(detail, dict):
                ...
                err_msg = (
                    detail[0].get("msg", str(detail[0])) if detail else str(detail)
                )
            else:
                err_msg = str(detail)
...
class AegisClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 8):
        self.base_url = (
            base_url or os.environ.get("AEGIS_BACKEND_URL", "http://127.0.0.1:8000")
        ).rstrip("/")
    ...
    def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AegisAPIError(
                "Authentication Error", status_code=res.status_code, detail=detail
            )
        except requests.RequestException as e:
            logger.exception(f"Connection error during login for {username}: {e}")
            raise AegisAPIError(
                "Backend microservice offline on port 8000. Please start the FastAPI server first.",
                detail=str(e),
            )
    ...
    def register(self, username: str, password: str, role: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise AegisAPIError(
                "Registration Error", status_code=res.status_code, detail=detail
            )
        except requests.RequestException as e:
            logger.exception(
                f"Connection error during registration for {username}: {e}"
            )
            raise AegisAPIError(
                "Backend microservice offline on port 8000.", detail=str(e)
            )
    ...
    def get_dashboard_summary(self, token: str) -> Dict[str, Any]:
        ...
        try:
            res = self.session.get(
                url, headers=self._get_headers(token), timeout=self.timeout
            )
            if res.status_code == 200:
                return res.json()
            return {}
    ...
    def chat_copilot(self, query: str, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            res = self.session.post(
                url, json=payload, headers=self._get_headers(token), timeout=12
            )
            if res.status_code == 200:
                return res.json()
            return {
                "reply": f"Unable to connect to AI Copilot inference backend. ERROR {res.status_code}: {res.text}"
            }
        except requests.RequestException as e:
