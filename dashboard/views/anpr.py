...
def render_anpr_page(client):
    ...
    with col1:
        st.markdown(
            '<div class="t-section" style="margin-bottom:8px;">Live Plate Reader Stream</div>',
            unsafe_allow_html=True,
        )
        search_query = st.text_input(
            "🔍 Search License Plate Number",
            placeholder="e.g. DL01-AB-1234",
            key="anpr_search_input",
        )

        data = [
            {
                "Timestamp": "14:35:12",
                "Plate Number": "DL01-AB-1234",
                "Vehicle Class": "Sedan",
                "Confidence": "98.4%",
                "Flag": "CLEAN",
            },
        ...
        if search_query:
            df_anpr = df_anpr[
                df_anpr["Plate Number"].str.contains(search_query.upper(), case=False)
            ]
        st.dataframe(df_anpr, use_container_width=True)

    with col2:
        st.markdown(
            '<div class="t-section" style="margin-bottom:8px;">ANPR Detection Metrics</div>',
            unsafe_allow_html=True,
        )
        st.markdown(
            metric_tile("Plates Scanned (24h)", "12,450", "", "#00f0ff", "📷"),
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            metric_tile("OCR Accuracy", "98.7", "%", "#10b981", "🎯"),
            unsafe_allow_html=True,
        )
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown(
            metric_tile("Hotlist Hits", "14", " matches", "#ef4444", "🚨"),
            unsafe_allow_html=True,
        )
