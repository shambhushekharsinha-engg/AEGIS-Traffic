...
def render_digital_twin_page(client):
    ...
    for _ in range(num_vehicles):
        data.append({
            ...
            "color": [random.randint(0, 255), 240, 255, 200] if random.random() > 0.2 else [255, 0, 80, 200], # Cyan/Blue or Red for emergency
    ...
    with col1:
        st.metric(label="Active V2X Nodes", value=f"{random.randint(800, 1500)}", delta="+12")
    with col2:
        st.metric(label="ReID Match Rate", value="94.2%", delta="0.5%")
    with col3:
        st.metric(label="Federated Syncs", value=f"{random.randint(40, 90)}/hr", delta="Active", delta_color="normal")
