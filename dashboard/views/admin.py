...
def render_admin_page(client):
    ...
    if st.button("⛓️ ANCHOR LEDGER SNAPSHOT TO BLOCKCHAIN", use_container_width=True, type="secondary"):
        try:
            token = st.session_state.get("jwt_token", "")
            headers = {"Authorization": f"Bearer {token}"}
            res = requests.post(
                f"{BACKEND}/api/v1/nextgen/blockchain-anchor",
                json={"payload": {"records_count": len(logs)}, "anchor_type": "AUDIT_LOG"},
                headers=headers, timeout=10
            )
            if res.ok:
                data = res.json()
                st.success(f"✅ SNAPSHOT ANCHORED IMMUTABLY.")
                st.code(f"TX HASH: {data['transaction_hash']}\\nBLOCK HEIGHT: {data['block_height']}\\nNETWORK: {data['network']}", language="bash")
            else:
