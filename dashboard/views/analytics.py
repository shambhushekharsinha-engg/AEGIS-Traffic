...
def render_analytics_page(client):
    ...
    with an_tab_upload:
        ...
        uploaded_file = st.file_uploader("Choose a traffic dataset file (CSV or JSON)", type=["csv", "json"], key="dataset_analyzer_uploader")
        if uploaded_file is not None:
            try:
                ...
                st.success(f"✅ Successfully loaded '{uploaded_file.name}' — {len(df_up)} rows, {len(df_up.columns)} columns.")

                c1, c2, c3 = st.columns(3)
                c1.markdown(metric_tile("Total Records", len(df_up), " rows", "#00f0ff", "📊"), unsafe_allow_html=True)
                c2.markdown(metric_tile("Total Fields", len(df_up.columns), " cols", "#a855f7", "📋"), unsafe_allow_html=True)
                c3.markdown(metric_tile("Memory Usage", f"{uploaded_file.size / 1024:.1f}", " KB", "#10b981", "💾"), unsafe_allow_html=True)

                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown('<div class="t-section" style="margin-bottom:8px;">Dataset Preview</div>', unsafe_allow_html=True)
                st.dataframe(df_up.head(10), use_container_width=True)

                st.markdown('<div class="t-section" style="margin-top:16px;margin-bottom:8px;">Descriptive Statistics</div>', unsafe_allow_html=True)
