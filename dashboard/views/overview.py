...
from dashboard.components.widgets import metric_tile, sec_div, mini_separator, risk_color
...
def render_overview_page(client):
    ...
    with vru_col:
        try:
            _vru_res = requests.get(f"{BACKEND}/api/v1/vru/crosswalk", params={"pedestrians": 3}, timeout=5)
            if _vru_res.ok:
                _vru = _vru_res.json()
                st.markdown(f"""
    ...
    with anpr_col:
        try:
            _anpr = requests.get(
                ...
                params={"latitude": st.session_state.get("latitude", 28.6315),
            ...
            _cc     = _anpr.get("summary", {}).get("country_flag", st.session_state.get("country_flag", "🇮🇳"))
            if _plates:
                for _p in _plates[:5]:
                    st.markdown(
    ...
    with viol_col:
        try:
            ...
            if _vlist:
                for _v in _vlist[:5]:
                    _fine_disp = _v.get("fine_local", f"{st.session_state.get('currency_symbol', '₹')}{_v.get('fine_amount','—')}")
                    _flag_v    = _v.get("country_flag", st.session_state.get("country_flag", "🇮🇳"))
                    st.markdown(
